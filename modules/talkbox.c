...
static void lpc_durbin(UTFLOAT *r, int p, float *k, float *g)
{
  int i, j;
  UTFLOAT a[ORD_MAX], at[ORD_MAX], e=r[0];
...
{
    UTFLOAT z[ORD_MAX], r[ORD_MAX], k[ORD_MAX], G, x;
    uint32_t i, j, nn=n;
    UTFLOAT min;
...
int ut_talkbox_init(ut_data *ut, ut_talkbox *p)
{
    ...
    n = (uint32_t)(0.01633f * ut->sr);
    if(n > UT_TALKBOX_BUFMAX) n = UT_TALKBOX_BUFMAX;

    /* calculate hanning window */
    if(n != p->N) {
        p->N = n;
        UTFLOAT dp = TWO_PI / (UTFLOAT)p->N;
        UTFLOAT pos = 0.0f;
        for(n=0; n < p->N; n++) {
            p->window[n] = 0.5f - 0.5f * (UTFLOAT)cos(pos);
            pos += dp;
        }
    }
    ...
    memset(p->buf0, 0, UT_TALKBOX_BUFMAX * sizeof(UTFLOAT));
    memset(p->buf1, 0, UT_TALKBOX_BUFMAX * sizeof(UTFLOAT));
    memset(p->car0, 0, UT_TALKBOX_BUFMAX * sizeof(UTFLOAT));
    memset(p->car1, 0, UT_TALKBOX_BUFMAX * sizeof(UTFLOAT));
    return UT_OK;
}

int ut_talkbox_compute(ut_data *ut, ut_talkbox *t, UTFLOAT *src, UTFLOAT *exc, UTFLOAT *out)
{
    int32_t p0=t->pos, p1 = (t->pos + t->N/2) % t->N;
    UTFLOAT e=t->emphasis, w, o, x, fx=t->FX;
    UTFLOAT p, q, h0=0.3f, h1=0.77f;
    UTFLOAT den;

    t->O = (uint32_t)((0.0001f + 0.0004f * t->quality) * ut->sr);
